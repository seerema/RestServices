...
public final class TypeResolver {
  ...
  static {
    JAVA_VERSION = Double
        .parseDouble(System.getProperty("java.specification.version", "0"));

    try {
      Unsafe unsafe = AccessController
          .doPrivileged(new PrivilegedExceptionAction<Unsafe>() {
            @Override
      ...
      GET_CONSTANT_POOL_METHOD_AT =
          constantPoolClass.getDeclaredMethod("getMethodAt", int.class);
   ...
   */
  public static <T, S extends T> Class<?> resolveRawArgument(Class<T> type,
      Class<S> subType) {
   ...
   */
  public static Class<?> resolveRawArgument(Type genericType,
      Class<?> subType) {
    ...
    if (arguments.length != 1)
      throw new IllegalArgumentException(
          "Expected 1 argument for generic type " + genericType +
   ...
   *         declared
   */
  public static <T, S extends T> Class<?>[] resolveRawArguments(Class<T> type,
      Class<S> subType) {
  ...
  public static <T, S extends T> Type reify(Class<T> type, Class<S> context) {
    return reify(resolveGenericType(type, context),
        getTypeVariableMap(context, null));
  }
  ...
  public static Class<?>[] resolveRawArguments(Type genericType,
    ...
    if (genericType instanceof ParameterizedType) {
      ...
      TypeVariable<?>[] typeParams =
          ((Class<?>) genericType).getTypeParameters();
  ...
  private static Class<?> resolveRawClass(Type genericType, Class<?> subType,
    ...
    if (genericType instanceof Class) {
      ...
      return resolveRawClass(((ParameterizedType) genericType).getRawType(),
          subType, functionalInterface);
    } else if (genericType instanceof GenericArrayType) {
      GenericArrayType arrayType = (GenericArrayType) genericType;
      Class<?> component = resolveRawClass(arrayType.getGenericComponentType(),
          subType, functionalInterface);
      return Array.newInstance(component, 0).getClass();
    } else if (genericType instanceof TypeVariable) {
      TypeVariable<?> variable = (TypeVariable<?>) genericType;
      genericType =
          getTypeVariableMap(subType, functionalInterface).get(variable);
  ...
  private static Type reify(final Type genericType,
    ...
    if (genericType == null)
      ...
      return reify(genericType, typeVariableTypeMap,
          new HashMap<ParameterizedType, ReifiedParameterizedType>());
  }
  ...
  private static Type reify(Type genericType,
    ...
    if (genericType instanceof ParameterizedType) {
      ...
      final ReifiedParameterizedType result =
          new ReifiedParameterizedType(parameterizedType);
      ...
      final Type reifiedComponentType = reify(
          genericArrayType.getGenericComponentType(), typeVariableMap, partial);
      ...
      throw new UnsupportedOperationException(
          "Attempted to reify wildcard type with name '" +
              wildcardType.getTypeName() + "' which has " + upperBounds.length +
              " upper bounds and " + lowerBounds.length +
    ...
    throw new UnsupportedOperationException("Reification of type with name '" +
        genericType.getTypeName() + "' and " + "class name '" +
        genericType.getClass().getName() + "' is not implemented.");
  }

  private static Map<TypeVariable<?>, Type> getTypeVariableMap(
      final Class<?> targetType, Class<?> functionalInterface) {
    Reference<Map<TypeVariable<?>, Type>> ref =
        TYPE_VARIABLE_CACHE.get(targetType);
    ...
    if (map == null) {
      ...
      if (CACHE_ENABLED)
        TYPE_VARIABLE_CACHE.put(targetType,
    ...
    for (Type type : types) {
      if (type instanceof ParameterizedType) {
        ...
        if (rawType instanceof Class)
          populateSuperTypeArgs(((Class<?>) rawType).getGenericInterfaces(),
              map, depthFirst);
        if (depthFirst)
          populateTypeArgs(parameterizedType, map, depthFirst);
   ...
   */
  private static void populateTypeArgs(ParameterizedType type,
      Map<TypeVariable<?>, Type> map, boolean depthFirst) {
   ...
   */
  private static void populateLambdaArgs(Class<?> functionalInterface,
      final Class<?> lambdaType, Map<TypeVariable<?>, Type> map) {
    if (RESOLVES_LAMBDAS) {
      // Find SAM
      for (Method m : functionalInterface.getMethods()) {
        if (!isDefaultMethod(m) && !Modifier.isStatic(m.getModifiers()) &&
            !m.isBridge()) {
          // Skip methods that override Object.class
          Method objectMethod = OBJECT_METHODS.get(m.getName());
          if (objectMethod != null && Arrays.equals(m.getTypeParameters(),
              objectMethod.getTypeParameters()))
            continue;
          ...
          Class<?>[] arguments =
              member instanceof Method ? ((Method) member).getParameterTypes()
  ...
  private static Member getMemberRef(Class<?> type) {
    ...
    for (int i = getConstantPoolSize(constantPool) - 1; i >= 0; i--) {
      ...
      if (member == null ||
          (member instanceof Constructor && member.getDeclaringClass().getName()
  ...
  private static boolean isAutoBoxingMethod(Method method) {
    ...
    return method.getName().equals("valueOf") && parameters.length == 1 &&
